...
// ---------- Staff Dashboard ----------
document.getElementById("staffDashboardBtn").addEventListener("click", function() {
  document.getElementById("complaintSection").style.display = "none";
  document.getElementById("staffSection").style.display = "block";
});

document.getElementById("loadComplaintsBtn").addEventListener("click", async function() {
  ...
  try {
    ...
    if (res.ok) {
      displayComplaints(data.complaints, staffId);
    } else {
...
function displayComplaints(complaints, staffId) {
  const listDiv = document.getElementById("complaintsList");
  listDiv.innerHTML = "";

  if (complaints.length === 0) {
    listDiv.innerHTML = "<p>No pending complaints.</p>";
    return;
  }
  ...
  complaints.forEach(complaint => {
    ...
    div.innerHTML = `
      ...
      <p><strong>Time:</strong> ${new Date(complaint.created_at).toLocaleString()}</p>
      <button onclick="completeComplaint(${complaint.id}, ${staffId})">Mark as Completed</button>
    `;
    listDiv.appendChild(div);
  });
}

async function completeComplaint(complaintId, staffId) {
  try {
    ...
    if (res.ok) {
      ...
      // Reload complaints
      document.getElementById("loadComplaintsBtn").click();
    } else {
